...
# %% auto 0
__all__ = ['DEFAULT_WLDM_MAPPING', 'SleepClassifierMode', 'SleepWakeClassifier', 'SGDLogisticRegression', 'MOResUNetPretrained',
...
DEFAULT_WLDM_MAPPING = {
    # Assumes N4 stage is present
    range(1, 3): 1, # light sleep
    range(3, 5): 2, # deep sleep
    range(5, 6): 3 # REM
}
...
class MOResUNetPretrained(SleepWakeClassifier):
    ...
    def get_needed_X_y_from_pair(self, pair: Tuple[DataSetObject, str], wldm_mapping: dict = DEFAULT_WLDM_MAPPING) -> Tuple[np.ndarray, np.ndarray] | None:
        """
        ...
        return self.get_needed_X_y(data_set, id, wldm_mapping)
    
    def get_needed_X_y(self, data_set: DataSetObject, id: str, wldm_mapping: dict=DEFAULT_WLDM_MAPPING) -> Tuple[np.ndarray, np.ndarray] | None:
        ...
        return mirrored_spectro, psg_to_WLDM(psg, wldm_mapping)
